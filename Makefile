...
ifeq ($(NO_DOCKER), 1)
  ...
  DOCKER_CMD := $(ENGINE) run --env GO111MODULE=$(GO111MODULE) --env GOFLAGS=$(GOFLAGS) --rm -v "$(PWD)":/go/src/github.com/uccps-samples/machine-api-operator:Z  -w /go/src/github.com/uccps-samples/machine-api-operator openshift/origin-release:golang-1.16
  ...
  # DOCKER_CMD := $(ENGINE) run --env GO111MODULE=$(GO111MODULE) --env GOFLAGS=$(GOFLAGS) --rm -v "$(PWD)":/go/src/github.com/uccps-samples/machine-api-operator:Z -w /go/src/github.com/uccps-samples/machine-api-operator registry.ci.openshift.org/ocp/builder:rhel-8-golang-1.15-uccp-4.6
