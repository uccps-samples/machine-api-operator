...
ifeq ($(NO_DOCKER), 1)
  ...
  DOCKER_CMD := $(ENGINE) run --env GO111MODULE=$(GO111MODULE) --env GOFLAGS=$(GOFLAGS) --rm -v "$(PWD)":/go/src/github.com/uccps-samples/machine-api-operator:Z  -w /go/src/github.com/uccps-samples/machine-api-operator $(BUILD_IMAGE)
  ...
  # DOCKER_CMD := $(ENGINE) run --env GO111MODULE=$(GO111MODULE) --env GOFLAGS=$(GOFLAGS) --rm -v "$(PWD)":/go/src/github.com/uccps-samples/machine-api-operator:Z -w /go/src/github.com/uccps-samples/machine-api-operator registry.ci.openshift.org/ocp/builder:rhel-8-golang-1.17-uccp-4.10
